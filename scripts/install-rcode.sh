...
case "$OS" in
  Linux)
    case "$ARCH" in
      x86_64)  ARTIFACT="rcode-linux-x86_64" ;;
      aarch64|arm64) ARTIFACT="rcode-linux-aarch64" ;;
      *)       err "Unsupported Linux architecture: $ARCH" ;;
    esac
    ;;
  Darwin)
    case "$ARCH" in
      arm64)   ARTIFACT="rcode-macos-aarch64" ;;
      x86_64)  ARTIFACT="rcode-macos-x86_64" ;;
...
if [ "$IS_WINDOWS" = true ]; then
  EXE=".exe"
  builds_dir="$LOCALAPPDATA/rcode/builds"
else
  EXE=""
  builds_dir="$HOME/.rcode/builds"
fi
stable_dir="$builds_dir/stable"
current_dir="$builds_dir/current"
version_dir="$builds_dir/versions"
launcher_path="$INSTALL_DIR/rcode${EXE}"
